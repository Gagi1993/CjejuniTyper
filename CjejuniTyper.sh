...
HS Serotype Complexes
  ...
  HS4c     — HS4 complex: HS4, HS13, HS16, HS43, HS50, HS62, HS64, HS65
...
Optional:
  -hs         HS full-locus BLAST database directory [default: databases/new_hs_db/blast]
...
LOS Typing Logic:
  ...
  - Threshold cascade: STRICT_1 (99.5% ID, 96% cov), STRICT_2 (99.0% ID, 95% cov), STRICT_3 (97% ID, 93% cov), STRICT_4 (95% ID, 93% cov), MID_1 (94% ID, 80% cov), MID_2 (92% ID, 80% cov), LOOSE (90% ID, 60% cov)
  - True mosaics retained: I/J, I/S (both hits independently >90% coverage)
  - LOS-B window fallback thresholds: STRICT (99.5% ID, 99% cov), MID (95% ID, 80% cov), LOOSE (80% ID, 60% cov)
...
HS Penner Serotype Complexes and Standalone Types:
  ...
  ├─────────────┼──────────────────────────────────────────────────────────────────┼─────────────────────────────────────┤
  │ HS4c        │ HS4, HS13, HS16, HS43, HS50, HS62, HS64, HS65             │ Mu_HS4B PCR → HS4c(HS16/64*)        │
  ...
  │ HS15c       │ HS15, HS58                                                  │ Mu_HS58 PCR → HS15c(HS58*)          │
  │ HS33_35c    │ HS33, HS35                                                        │  │
  ...
  HS37, HS38, HS40, HS41, HS42, HS52, HS53, HS55, HS57, HS63
...
if [ "$INSTALL" -eq 1 ]; then
  ...
  if $PKG_MANAGER env list | grep -q "^${ENV_NAME}"; then
    ...
    $PKG_MANAGER create -y -n "$ENV_NAME" \
      ...
      blast ncbi-amrfinderplus exonerate seqkit mash
    echo ""
...
END{
  ...
  for(s in samples){
    ...
    for(t=1;t<=7;t++){
      ...
      cov_out = sprintf("%.1f", (b_cov > 1 ? 100 : b_cov*100))
...
for f in "${TARGET_FASTAS[@]}"; do
  ...
  (
    ...
    cstii_fasta="$VF_DB/cstII/cstII.fasta"

    hit=$(run_blastn \
      -query "$cstii_fasta" \
      -subject "$f" \
      -strand both \
      -outfmt "6 qstart qend pident length qlen bitscore qseq sseq" \
      -evalue 1e-10 \
      -max_hsps 1 \
      2>/dev/null \
      | awk -v pid="$VF_PID" -v pcov="$VF_PCOV" '
    ...
    if [ -z "$hit" ]; then
      ...
      read -r qstart qend best_pid_v aln_len qlen bitscore qseq sseq <<< "$hit"

      pid_r=$(awk -v x="$best_pid_v" \
        'BEGIN {printf "%d", int(x+0.5)}')

      cov_r=$(awk -v a="$aln_len" -v q="$qlen" \
        'BEGIN {printf "%d", int((a/q)*100+0.5)}')

      codon51=$(python3 - "$qstart" "$qend" "$qseq" "$sseq" <<'PY'
import sys

qstart = int(sys.argv[1])
qend = int(sys.argv[2])
qseq = sys.argv[3].upper()
sseq = sys.argv[4].upper()

step = 1 if qend >= qstart else -1
qpos = qstart
bases = {}
...
for reference_base, genome_base in zip(qseq, sseq):
    if reference_base != "-":
        if qpos in (151, 152, 153):
            if genome_base not in "ACGT":
                base = "N"
            elif step == 1:
                base = genome_base
            else:
                base = complement[genome_base]

            bases[qpos] = base

        qpos += step

print("".join(bases.get(position, "N") for position in (151, 152, 153)))
PY
)
